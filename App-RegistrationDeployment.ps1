...
If ($($UrlStatusCode = Get-UrlStatusCode -Url "$($ConfigurationData.RedistPath)/WebSite.json") -ne 200)                         { $Message = "Template file location could not be verified`n$($ConfigurationData.RedistPath)/WebSite.json`n$($UrlStatusCode)" ; Write-Warning $Message ; $ParamValidation = $False ; Try { Invoke-Logger -Message $Message -Severity E -Category "AzureRmResourceGroupDeployment" } Catch {}}
If ($($UrlStatusCode = Get-UrlStatusCode -Url "$($ConfigurationData.RedistPath)/$($ConfigurationData.WebApplication)") -ne 200) { $Message = "Web application file location could not be verified`n$($ConfigurationData.RedistPath)/$($ConfigurationData.WebApplication)`n$($UrlStatusCode)" ; Write-Warning $Message ; $ParamValidation = $False ; Try { Invoke-Logger -Message $Message -Severity E -Category "AzureRmResourceGroupDeployment" } Catch {}}
