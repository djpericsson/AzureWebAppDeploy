...
If ($($UrlStatusCode = Get-UrlStatusCode -Url "$($ConfigurationData.RedistPath)/WebSite.json") -ne 200)                         { $Message = "Template file location could not be verified" ; Write-Warning $Message ; $ParamValidation = $False ; Try { Invoke-Logger -Message "$Message`nUrl: $($ConfigurationData.RedistPath)/WebSite.json`nStatusCode: $($UrlStatusCode)" -Severity E -Category "AzureRmResourceGroupDeployment" } Catch {}}
If ($($UrlStatusCode = Get-UrlStatusCode -Url "$($ConfigurationData.RedistPath)/$($ConfigurationData.WebApplication)") -ne 200) { $Message = "Web application file location could not be verified" ; Write-Warning $Message ; $ParamValidation = $False ; Try { Invoke-Logger -Message "$Message`nUrl: $($ConfigurationData.RedistPath)/$($ConfigurationData.WebApplication)`nStatusCode: $($UrlStatusCode)" -Severity E -Category "AzureRmResourceGroupDeployment" } Catch {}}
