...
If ($UrlStatusCode -ne 200)                         { $Message = "Template file location could not be verified" ; Write-Warning $Message ; $ParamValidation = $False ; Try { Invoke-Logger -Message "Url: $($ConfigurationData.RedistPath)/WebSite.json : StatusCode: $($UrlStatusCode)" -Severity E -Category "AzureRmResourceGroupDeployment" } Catch {}}

$UrlStatusCode = Get-UrlStatusCode -Url  "$($ConfigurationData.RedistPath)/$($ConfigurationData.WebApplication)"
If ($UrlStatusCode -ne 200)                         { $Message = "Web application file location could not be verified" ; Write-Warning $Message ; $ParamValidation = $False ; Try { Invoke-Logger -Message "Url: $($ConfigurationData.RedistPath)/$($ConfigurationData.WebApplication) StatusCode: $($UrlStatusCode)" -Severity E -Category "AzureRmResourceGroupDeployment" } Catch {}}
