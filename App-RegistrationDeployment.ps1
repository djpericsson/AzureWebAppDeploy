...
If ($UrlStatusCode -ne 200)                         { $Message = "Web application file location could not be verified" ; Write-Warning $Message ; $ParamValidation = $False ; Try { Invoke-Logger -Message $Message,"Url: $($ConfigurationData.RedistPath)/$($ConfigurationData.WebApplication)","StatusCode: $($UrlStatusCode)" -Severity E -Category "AzureRmResourceGroupDeployment" } Catch {}}
